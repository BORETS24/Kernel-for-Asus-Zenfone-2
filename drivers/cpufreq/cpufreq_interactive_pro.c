...
static struct attribute *interactivep_attributes[] = {
...
static struct attribute_group interactivep_attr_group = {
	.attrs = interactivep_attributes,
...
static struct notifier_block cpufreq_interactivep_idle_nb = {
	.notifier_call = cpufreq_interactive_idle_notifier,
};

static int cpufreq_governor_interactive_pro(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_START:
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&interactivep_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_interactivep_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_interactivep_idle_nb);
...
struct cpufreq_governor cpufreq_gov_interactive_pro = {
	.name = "interactive_pro",
	.governor = cpufreq_governor_interactive_pro,
...
fs_initcall(cpufreq_interactive_pro_init);
#else
